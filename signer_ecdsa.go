...
func (s *EcSigner) SignTx(tx *schema.Transaction) error {
...
func VerifyEcdsaTxSig(tx *schema.Transaction) error {
	if tx.Signature == "" {
...
func GetEcdsaTxOwner(tx *schema.Transaction) (string, error) {
...
func recoverPubkey(tx *schema.Transaction) (*ecdsa.PublicKey, error) {
	if tx.Signature == "" {
